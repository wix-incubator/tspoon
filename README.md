...
Typescript transpilation is useally ```source -> AST -> target```. tspoon allowes you to apply vistors on the AST before it is transformed to target. The process will look like this ```source -> AST -visitors-> AST -> target```. This technique enables early optimizations and error detection for custom language features.
